    ...
    SN_MsgType* p_packet_type, word16* p_packet_id)
{
    ...
    if (p_packet_id) {
        switch(packet_type) {
            ...
            case SN_MSG_TYPE_PUBACK:
                /* octet 4-5 */
                MqttDecode_Num(rx_buf + 2, p_packet_id, rx_buf_len - (rx_buf + 2 - (rx_buf - 1)));
                break;
            case SN_MSG_TYPE_PUBCOMP:
            case SN_MSG_TYPE_PUBREC:
            case SN_MSG_TYPE_PUBREL:
            case SN_MSG_TYPE_UNSUBACK:
                /* octet 2-3 */
                MqttDecode_Num(rx_buf, p_packet_id, rx_buf_len - (rx_buf - (rx_buf - 1)));
                break;
            case SN_MSG_TYPE_SUBACK:
                /* octet 5-6 */
                MqttDecode_Num(rx_buf + 3, p_packet_id, rx_buf_len - (rx_buf + 3 - (rx_buf - 1)));
                break;
            case SN_MSG_TYPE_ADVERTISE:
            case SN_MSG_TYPE_SEARCHGW:
            case SN_MSG_TYPE_GWINFO:
...
int SN_Decode_Advertise(byte *rx_buf, int rx_buf_len, SN_Advertise *gw_info)
{
    ...
    if (gw_info != NULL) {
        ...
        rx_payload += MqttDecode_Num(rx_payload, &gw_info->duration, rx_buf_len - (rx_payload - rx_buf));
    }
    (void)rx_payload;
...
int SN_Decode_GWInfo(byte *rx_buf, int rx_buf_len, SN_GwInfo *gw_info)
{
    ...
    if (total_len == SN_PACKET_LEN_IND) {
        /* The length is stored in the next two bytes */
        rx_payload += MqttDecode_Num(rx_payload, (word16*)&total_len, rx_buf_len - (rx_payload - rx_buf));
    }
...
int SN_Decode_Register(byte *rx_buf, int rx_buf_len, SN_Register *regist)
{
    ...
    if (regist != NULL) {
        ...
        rx_payload += MqttDecode_Num(rx_payload, &regist->topicId, rx_buf_len - (rx_payload - rx_buf));

        /* Decode packet ID */
        rx_payload += MqttDecode_Num(rx_payload, &regist->packet_id, rx_buf_len - (rx_payload - rx_buf));
...
int SN_Decode_RegAck(byte *rx_buf, int rx_buf_len, SN_RegAck *regack)
{
    ...
    if (regack != NULL) {
        /* Decode Topic ID assigned by GW */
        rx_payload += MqttDecode_Num(rx_payload, &regack->topicId, rx_buf_len - (rx_payload - rx_buf));

        /* Decode packet ID */
        rx_payload += MqttDecode_Num(rx_payload, &regack->packet_id, rx_buf_len - (rx_payload - rx_buf));
        ...
        SN_SubAck *subscribe_ack)
{
    ...
    if (subscribe_ack) {
        ...
        rx_payload += MqttDecode_Num(rx_payload, &subscribe_ack->topicId, rx_buf_len - (rx_payload - rx_buf));
        rx_payload += MqttDecode_Num(rx_payload, &subscribe_ack->packet_id, rx_buf_len - (rx_payload - rx_buf));
...
int SN_Decode_Publish(byte *rx_buf, int rx_buf_len, SN_Publish *publish)
{
    ...
    if (total_len == SN_PACKET_LEN_IND) {
        /* The length is stored in the next two bytes */
        rx_payload += MqttDecode_Num(rx_payload, &total_len, rx_buf_len - (rx_payload - rx_buf));
    }
    ...
    rx_payload += MqttDecode_Num(rx_payload, &publish->packet_id, rx_buf_len - (rx_payload - rx_buf));
    ...
    SN_PublishResp *publish_resp)
{
    ...
    if (publish_resp) {
        if (type == SN_MSG_TYPE_PUBACK) {
            rx_payload += MqttDecode_Num(rx_payload, &publish_resp->topicId, rx_buf_len - (rx_payload - rx_buf));
        }

        rx_payload += MqttDecode_Num(rx_payload, &publish_resp->packet_id, rx_buf_len - (rx_payload - rx_buf));
        ...
        SN_UnsubscribeAck *unsubscribe_ack)
{
    ...
    /* Decode SubAck fields */
    if (unsubscribe_ack) {
        rx_payload += MqttDecode_Num(rx_payload, &unsubscribe_ack->packet_id, rx_buf_len - (rx_payload - rx_buf));
    }
    (void)rx_payload;
    ...
    switch (client->packet.stat)
    {
        case MQTT_PK_BEGIN:
        {
            ...
            if (rx_buf[0] == SN_PACKET_LEN_IND){
                ...
                (void)MqttDecode_Num(&rx_buf[1], &total_len, rx_buf_len - 1);
