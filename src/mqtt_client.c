...
/* [MQTT-2.3.1-1] / MQTT 3.1.1 section 2.3.1: a new QoS-related Control
 * Packet must use a Packet Identifier that is not currently in use; the
 * value only becomes reusable after the corresponding acknowledgement
 * flow completes. This is the central allocator implementing that rule.
 *
 * The pending response list (under WOLFMQTT_MULTITHREAD) is the
 * authoritative in-use set: an entry is present from the moment a
 * QoS-related packet is registered and is removed only after the matching
 * ack is processed. We walk it to skip any value that is still in flight
 * and advance the per-client counter to find the next unused id.
 *
 * Under WOLFMQTT_MULTITHREAD lockClient is held across both the counter
 * advance and the in-flight walk. Splitting them would allow two
 * concurrent callers to increment past the same pre-value, see an empty
 * resp list (neither has registered yet), and both return the same id —
 * the very collision this allocator exists to prevent.
 *
 * Returns 0 when client is NULL, lockClient cannot be acquired, or the
 * full 1..0xFFFF range is in flight. In all three cases the caller must
 * back off rather than reuse a value. */
word16 MqttClient_NextPacketId(MqttClient *client)
{
    word32 tries;
    word16 result = 0;
...
#ifdef WOLFMQTT_MULTITHREAD
    /* Lock failure: surface as the documented sentinel. Returning an
     * unverified candidate would silently violate the in-use guarantee. */
    if (wm_SemLock(&client->lockClient) != 0) {
        return 0;
    }
#endif

    for (tries = 0; tries < (word32)MAX_PACKET_ID + 1U; tries++) {
        word16 candidate;

        client->next_packet_id++;
        if (client->next_packet_id == 0) {
            client->next_packet_id = 1; /* skip 0 on wrap */
        }
...
#ifdef WOLFMQTT_MULTITHREAD
        {
            ...
            for (tmpResp = client->firstPendResp;
                 ...
                 tmpResp = tmpResp->next)
            {
                if (tmpResp->packet_id == candidate) {
                    collision = 1;
                    break;
                }
            }
